...
# (400 / (攻略率(e2) / 10 + 10 - 16m以内の敵数))秒毎に召喚
    ...
    # 周囲に敵が全くいない場合カウントが加速(x2)
        execute if entity @e[type=#lib:living_without_player,tag=Enemy,distance=..32] unless entity @e[type=#lib:living_without_player,tag=Enemy.Boss,distance=..64,limit=1] run scoreboard players operation $MaxEnemies Temporary *= $2 Const
    # NightmareCount
